...
std::ostream& operator<<(std::ostream& os, const Car& car){
    ...
    if(!cur){
        fastCache = 0;
        os<< driverName <<"--" << Laptime(fastCache) << "--" << Laptime(fastCache) << "--" << Laptime(fastCache) << std::endl;
        return os;
    }
    if(cur && !(cur->getNext())){
        ...
        //std::cout << fastCache<< "   ";
        os<< driverName <<"--" << Laptime(fastCache) << "--" << Laptime(fastCache) << "--" << Laptime(total) << std::endl;
        return os;
    }
