...
class Server():
	def __init__(self,address,port,repeaterAddress,inOrderReceiver):
		...
		self.inOrderReceiver = inOrderReceiver # whether receives inorder or reverse order

	def separateDataPacket(self,receivedPacket):
		#unwrap headers and seperate data
	...
	def serverFunction(self):
		...
		receiving_sock.settimeout(0.1) #important for resends
			...
			try:
				...
				if receivedPacket:
					...
					if(self.checkForChecksum(receivedPacket)):
						...
						with serverAccessLock:
							#lock to ensure safety of global variables
							if(receivedBefore == False):
								#keep track of first arrival
								self.receivedBefore = True
								self.firstReceivalTime = sendingTime
							self.lastReceivalTime = time.time()
							#checks if other thread has finished before us
							if stillRunning == False:
								break
							if forwardGoingIndex > backwardGoingIndex:
								stillRunning = False
								break
							#no stopping condition achieved , just add it to respective data array
				...
				with serverAccessLock:
					...
					if(forwardGoingIndex > backwardGoingIndex):
						stillRunning = False
						break
					#no stopping condition , other thread didn't finish
					# resend last ack
					if self.inOrderReceiver:
						sendingPacket = self.createResponsePacket(forwardGoingIndex)
...
if __name__ == "__main__":
	...
	repeaterAddresses = ['10.10.1.1','10.10.2.2']
	localPort = 12000
