...
class Game():
	...
	def state(self):
		...
		#playersName = list(map(lambda x: {x.nickname}, self.players))
		playersName = ""
		for p in self.players:
			playersName += p.nickname + ","
		playersPos = list(map(lambda x: {x.nickname: x.position}, self.players))
		playerRound = list(map(lambda x: {x.nickname: x.cycle}, self.players))
		playerCredit = list(map(lambda x: {x.nickname: x.credit}, self.players))
		playerArt = list(map(lambda x: {x.nickname: x.pickedArt}, self.players))
		playerTurnType = list(map(lambda x: {x.nickname: x.type}, self.players))
		hasToWait = list(map(lambda x: {x.nickname: x.hasToWait}, self.players))
		cells = self.dict["cells"]
		return {"playersPos": playersPos, "hasToWait" : hasToWait, "credits": playerCredit, "negeldi": self.negeldi, "pickedArtifact": playerArt,
				"playerTurnType": playerTurnType, "playerroundno": playerRound, "currentTurn": self.currentTurn,
				"nextTurn": self.nextTurn, "playersName" : playersName, "winner": self.winner, "loser": self.loser, "gameOver": self.gameOver, "players": self.players, "cells": self.cells}
...
games2 = [game1.name, game2.name, game3.name, game4.name, game5.name, game6.name, game7.name]
...
def loadgames(request):
	return HttpResponse(dumps({'obj':games2}), 'text/json')

# her oyunun kendi sayfasi(details gibi)
def detail(request, gamename):
	return HttpResponse(dumps({'result':"Okey"}), 'text/json')
...
def ready(request, gamename, playerid):
	...
	#context = game.state()
	context = {}
	if game.currentTurn is not None:
		context["currentTurn"] = game.currentTurn.nickname
	context["playerid"] = playerid
	context["playername"] = player.nickname
	context["name"] = gamename
	context['result'] = "no"
	if game.started:
		context['result'] = "yes"
		return HttpResponse(dumps(context), 'text/json')
	return HttpResponse(dumps(context), 'text/json')
...
def checkready(request, gamename, playerid):
	...
	#context = {"playerid": playerid, "currentTurn": game.currentTurn, "players": game.players, "name": gamename}
	#context = game.state()
	print(list(map(lambda x:x.isReady,game.players)))
	print(game.started)
	context ={}
	context["playername"] = player.nickname
	if game.currentTurn is not None:
...
def turn(request, gamename, playerid):
	...
	context1 = game.state()
	context = {}
	# {"playersPos": playersPos, "credits": playerCredit, "negeldi": self.negeldi, "pickedArtifact": playerArt,
	# 			"playerTurnType": playerTurnType, "playerroundno": playerRound, "currentTurn": self.currentTurn,
	# 			"nextTurn": self.nextTurn, "winner": self.winner, "loser": self.loser, "gameOver": self.gameOver, "players": self.players, "cells": self.cells}



	#context["playersPos"] = dumps(context1['playersPos'])
	#context["credits"] = dumps(context1['credits'])
	context["negeldi"] = context1['negeldi']
	context["playerid"] = playerid
	if context1['nextTurn'] != None :
		context["nextTurn"] = dumps(context1['nextTurn'].nickname)
	if context1['winner'] != None:
		context["winner"] = dumps(context1['winner'].nickname)
	if context1['loser'] != None:
		context["loser"] = dumps(context1['loser'].nickname)
	context["gameOver"] = dumps(context1['gameOver'])
	if context1['currentTurn'] != None:
		context["currentTurn"] = dumps(context1['currentTurn'].nickname)
	context["playersTurnType"] = dumps(context1['playerTurnType'])
	context["playersRound"] = dumps(context1['playerroundno'])
	context["playersPos"] = dumps(context1['playersPos'])
	context["playersCredit"] = dumps(context1['credits'])
	context["playersTurnType"] = dumps(context1['playerTurnType'])
	context["playersArt"] = dumps(context1['pickedArtifact'])
	#context["players"] = ["emre","osman"]
	context["cells"] = dumps(context1['cells'])
	
	if len(game.players) == 1:
		
		context["gameOver"] = dumps(context1['gameOver'])
		context["winner"] = dumps(game.players[0].nickname)
		return HttpResponse(dumps(context), 'text/json')
	...
	if len(list(filter(lambda x: x.id == int(playerid), game.players))) == 0:
		...
		if context1['currentTurn'] != None:
			context["currentTurn"] = dumps(game.currentTurn.nickname)
		context["name"] = gamename
		context["playerid"] = playerid
		return HttpResponse(dumps(context), 'application/json')
		#return render(request, 'rulebasedgame/turn.html', context)
	...
	if request.method == "GET":
		...
		context["playername"] = player.nickname
		context["name"] = gamename
		context["hasToWait"] = dumps(context1['hasToWait'])
		context["playersTurnType"] = dumps(context1['playerTurnType'])
		context["playersRound"] = dumps(context1['playerroundno'])
		context["playersPos"] = dumps(context1['playersPos'])
		context["playersCredit"] = dumps(context1['credits'])
		...
		if player.hasToWait == 0:
			...
			# game.currentTurn = game.players[playerIndex+1]
				...
				if choice == "true" or choice == "True":
					context1 = game.pick(player, True)
					context = {}
					#context["playersPos"] = dumps(context1['playersPos'])
					#context["credits"] = dumps(context1['credits'])
					context["negeldi"] = context1['negeldi']
					...
					context1 = game.pick(player, False)
					context = {}
					#context["playersPos"] = dumps(context1['playersPos'])
					#context["credits"] = dumps(context1['credits'])
					context["negeldi"] = context1['negeldi']
