...
private:
	...
	unsigned char ofDetector(float lvalue,float rvalue);
	Vector3f ambientCalculate(Shape* obj);
	Vector3f diffuseCalculate(Shape* minObj,ReturnVal hitFinal, PointLight* light);
	Vector3f specularCalculate(Shape* minObj, ReturnVal hitFinal, PointLight* light, Ray primaryRay);
	int shadowCalculate(Shape* minObj, ReturnVal hitFinal, PointLight* light);
	Vector3f recursion(Ray primaryRay,int recDepth);
};
